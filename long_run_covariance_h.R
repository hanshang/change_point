kweights <- ftsa:::kweights
FlatTop <- ftsa:::FlatTop

# compute the estimated long-run covariance function

# dat: (p x n) data matrix
# C0: tuning parameter
# H: tuning parameter
...
long_run_covariance_estimation <- function (dat, C0 = 3, H = 3)
{
    ...
    h_opt_FT_BT = h_opt_fun(band_ini = T^(0.2), T = T, kern_type = "BT", 
                            kern_type_ini = "FT")
    return(list(hat_h_opt = h_opt_FT_BT$hat_h_opt, C_0_est = h_opt_FT_BT$C_0_est))
}
